...
class UserService:
    ...
    async def revoke_all_mobile_sessions(self, user_id: UUID) -> bool:
        ...
        revoked_count = await self.mobile_token_service.revoke_all_mobile_sessions(user_id)
        return revoked_count > 0

    async def get_mobile_device_statistics(self, user_id: UUID) -> dict:
        """Get mobile device statistics for user."""
        return await self.mobile_token_service.get_device_statistics(user_id)
